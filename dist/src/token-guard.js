...
export function decideAuthorityMode(snapshot) {
...
export function detectBlockReset(previous, current, config = { percentDropThreshold: 50, timeGapHours: 4 }) {
    if (!previous) {
...
export function recordCcusageShapeMiss(state, raw) {
...
export function isVerificationExceptionBudgetExceeded(budget) {
    if (!budget) {
        return false;
    }
...
function authorityModeForPercent(percent) {
    if (percent >= 92)
        return "halt";
    if (percent >= 70)
        return "claude-paused";
    if (percent >= 65)
        return "codex-leaning";
    if (percent >= 50)
        return "efficient";
    return "normal";
}
function decisionForMode(mode) {
    switch (mode) {
        case "halt":
...
function makeDecision(mode, allowClaudeAutonomous, allowCodex, allowCcusage, allowVerificationException, shouldHalt, reason) {
    return { mode, allowClaudeAutonomous, allowCodex, allowCcusage, allowVerificationException, shouldHalt, reason };
}
function blockWindowKey(snapshot) {
    const parts = [snapshot.startTime, snapshot.endTime, snapshot.resetTime].filter((value) => Boolean(value));
...
function sanitizeCcusageShape(raw) {
    try {
