...
async function routeCommand(args) {
    const taskKind = (args[0] ?? "implementation");
...
async function goalStartCommand(args) {
    const task = args.join(" ").trim() || "unspecified goal";
    await startGoal(task);
}
async function goalStartHookCommand() {
...
async function nextCommand() {
    ...
    if (ratchet.advanced)
        await writeFile(planPath, ratchet.content, "utf8");
...
async function verifyAndAdvanceCommand() {
    const result = await verifyAndAdvancePlan(cwd);
    console.log(result.evidence);
    process.exitCode = result.ok ? 0 : 2;
}
async function executeRoutedCommand(args) {
    const taskKind = (args[0] ?? "implementation");
...
async function metareviewAutoCommand(args) {
    const verifierEvidence = readFlag(args, "--evidence") ?? "Verifier evidence unavailable.";
    const completedBy = (readFlag(args, "--completed-by") ?? "codex");
...
async function compactBackupCommand(args) {
    const stdin = await readStdin();
...
async function statuslineCommand() {
    ...
    if (lineInput.contextPercent >= 85) {
        fireNotification(`Context at ${Math.round(lineInput.contextPercent)}%`, "Run /compact <focus hint> before continuing.", `context-${Math.round(lineInput.contextPercent)}`);
    }
...
async function guardPlanEditCommand() {
    const stdin = await readStdin();
    let input = {};
    try {
...
async function verifyClaudeFiles() {
    ...
    for (const file of commandFiles) {
        ...
        if (!encoding.ok)
            throw new Error(`commands/${file}: ${encoding.message}`);
        const content = bytes.toString("utf8");
        const result = validateClaudeCommandFrontmatter(content);
        if (!result.ok)
            throw new Error(`commands/${file}: ${result.message}`);
    ...
    for (const file of await safeReaddir(rulesDir)) {
        ...
        const encoding = validateUtf8NoBomLf(bytes);
        if (!encoding.ok)
            throw new Error(`rules/${file}: ${encoding.message}`);
        const content = bytes.toString("utf8");
        const result = validateRuleFrontmatter(content);
        if (!result.ok)
            throw new Error(`rules/${file}: ${result.message}`);
...
async function readCcusage() {
    return readCcusageData({ cwd, env: process.env });
}
async function readStatuslineUsage() {
    if (process.env.CCUSAGE_MOCK_JSON)
        return JSON.parse(process.env.CCUSAGE_MOCK_JSON);
    const cachePath = join(cwd, ".myorch", "cache", "statusline-ccusage.json");
    const cached = await readJson(cachePath);
    if (cached && typeof cached === "object" && "ts" in cached && Date.now() - Number(cached.ts) < 5_000) {
        return cached.value ?? {};
    }
...
function fireNotification(title, message, dedup) {
