...
export function parseCcusage(raw) {
    const warnings = [];
    const snapshot = { warnings, raw };
    ...
    if (claude !== undefined)
        snapshot.claudePercent = claude;
    if (codex !== undefined)
        snapshot.codexPercent = codex;
...
function percentFromModel(model) {
    if (!model)
        return undefined;
...
function percentFromActiveBlock(root) {
