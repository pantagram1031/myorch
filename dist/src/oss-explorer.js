...
export function evaluateOssCandidate(candidate, policy = {
    ...
    const protectedPackageSection = candidate.packageJsonSections?.find((section) => protectedPackageSections.has(section));
...
export async function loadOssProtectionPolicy(root) {
    try {
        ...
        return {
            ...
            protectedPackageJsonSections: Array.isArray(parsed.packageJsonSections) && parsed.packageJsonSections.every((item) => typeof item === "string")
...
function isDependencyOnlyPackageChange(candidate) {
    const packageSections = candidate.packageJsonSections ?? [];
    return (candidate.touchedPaths.length > 0 &&
        candidate.touchedPaths.every((path) => path === "package.json") &&
        packageSections.length > 0 &&
        packageSections.every((section) => section === "dependencies" || section === "optionalDependencies" || section === "peerDependencies"));
}
