...
export function validateFrontmatter(markdown) {
    return validateRuleFrontmatter(markdown);
}
export function validateRuleFrontmatter(markdown) {
    try {
...
export function validateUtf8NoBomLf(content) {
...
export function validateClaudeCommandFrontmatter(markdown) {
    try {
...
export function validateJsonSchema(schema, value) {
    ...
    if (errors.length === 0)
        return { ok: true, message: "json schema ok" };
...
export async function checkBashSyntax(scriptPath, bashPath = "C:\\Program Files\\Git\\bin\\bash.exe") {
    return runCommand(`"${bashPath}" -n "${scriptPath}"`);
}
export async function checkPowerShellSyntax(scriptPath) {
...
function validateJsonValue(schema, value, path) {
    ...
    if (schema.type === "object") {
        ...
        for (const [key, childSchema] of Object.entries(schema.properties ?? {})) {
            if (key in record)
                errors.push(...validateJsonValue(childSchema, record[key], `${path}/${key}`));
        }
    }
